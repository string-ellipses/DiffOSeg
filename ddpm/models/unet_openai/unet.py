...
from .promptblock import PromptBlock
import logging
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if stage == 2:
            self.input_blocks = nn.ModuleList(
                [TimestepEmbedSequential(conv_nd(dims, in_channels + num_reviewers, ch, 3, padding=1))]
            )
        else:
            self.input_blocks = nn.ModuleList(
        ...
        # self.ch_m = ch
        #self.prompt_blocks.append(PromptBlock(prompt_dim=128, prompt_len=5, prompt_size=96, lin_dim=ch, learnable_input_prompt=False))
        if stage == 2:
            self.prompt_block = PromptBlock(prompt_dim = 128, prompt_len=5, prompt_size=96, lin_dim=ch, middle_dim = ch, learnable_input_prompt=True) 
        ...
        #     PromptBlock(prompt_dim=128, prompt_len=5, prompt_size=96, lin_dim=(128 + 4), learnable_input_prompt=False)
        #     for _ in range(len(list(enumerate(channel_mult))[::-1]))
        # ])

        if self.sofmtax_output:
            # if stage == 2:
            #     self.out = nn.Sequential(
            #         normalization(ch + prompt_dim),
            #         nn.SiLU(),
            #         zero_module(conv_nd(dims, input_ch + prompt_dim, out_channels, 3, padding=1)),
            #         nn.Softmax(dim=1)
            #     )
            # else:
            self.out = nn.Sequential(
        ...
        # # prompt & fuse
        # num_conv_stage = 3
        # self.intermedia_prompt = nn.Parameter(th.randn(1, 2, patch_size[0]//16, patch_size[1]//32, patch_size[2]//32))
    ...
    def forward(self, x, input_condition, feature_condition, timesteps, y=None):
        ...
        image_condition = input_condition['hint']
        if self.stage == 2:
            reviewer_id = input_condition['txt']  # 假设这是一个批量的ID张量
            reviewer_one_hot = th.nn.functional.one_hot(reviewer_id, num_classes=self.num_reviewers).float()
        # 将图像条件和评估者one-hot编码拼接
        #combined_condition = th.cat([image_condition, reviewer_one_hot.unsqueeze(2).unsqueeze(3).expand(-1, -1, x.size(2), x.size(3))], dim=1)

        if self.stage == 2:
            x = th.cat([x, image_condition, reviewer_one_hot.unsqueeze(2).unsqueeze(3).expand(-1, -1, x.size(2), x.size(3))], dim=1)
        else:
            x = th.cat([x, image_condition], dim=1)
        ...
        if self.stage == 2:
            h = self.prompt_block(h, reviewer_id, timesteps)
        ...
        for idx, module in enumerate(self.output_blocks):
            ...
            h = module(h, emb, context)
              # 添加 PromptBlock
                # h = th.cat([h, prompt_h], dim=1)  # 将 PromptBlock 输出与当前特征拼接 
                # reduce
                     
        h = h.type(x.dtype)

        # if self.stage == 2:
        #     prompt_h= self.prompt_block(h, reviewer_one_hot)

        #     h = th.cat([h, prompt_h], dim=1)
        # output handling
