...
def archive_code(path: str) -> None:
    # 要归档的文件列表
    files_to_archive = ["evaluation/evaluate_lidc_uncertainty_2.py", "ddpm/models/unet_openai/unet.py", "params_eval.yml"]  # 在这里指定要保存的文件路径

    # Copy params.yml to the specified path
    shutil.copy("params.yml", path)
    
    # Create a tar.gz archive with only the specified files
    files = ' '.join(files_to_archive)  # 将文件列表拼接成一个字符串
    os.system(f"tar -czf {os.path.join(path, 'code.tar.gz')} {files}")
