...
class DenoisingModel(nn.Module):
    ...
    def forward(self, x: Tensor, condition: dict, feature_condition: Tensor = None, t: Optional[Tensor] = None, label_ref_logits: Optional[Tensor] = None,
    ...
    def forward_step(self, x: Tensor, condition: dict, feature_condition: Tensor, t: Tensor) -> Tensor:
        return self.unet(x, condition, feature_condition=feature_condition, timesteps=t)

    def forward_denoising(self, x: Optional[Tensor], condition: dict, feature_condition: Tensor, init_t: Optional[int] = None,
...
class DenoisingControlModel(nn.Module):
    ...
    def forward(self, x: Tensor, condition: Tensor, control: Tensor = None, t: Optional[Tensor] = None, label_ref_logits: Optional[Tensor] = None,
                validation: bool = False) -> Union[Tensor, dict]:
        ...
        # #     if not isinstance(x, Tensor):
        # #         raise ValueError("'x' needs to be a Tensor at training time")
        #     return self.forward_step(x, condition, control, t) 
        # else:
        #     if validation:
        #         return self.forward_step(x, control, t)
        if t is None:
            return self.forward_denoising(x, condition, control, t, label_ref_logits=label_ref_logits)
        else:
            return self.forward_step(x, condition, control, t) 

    def forward_step(self, x: Tensor, condition: Tensor, control: Tensor, t: Tensor) -> Tensor:
        return self.unet(x, condition, control, timesteps=t)

    def forward_denoising(self, x: Optional[Tensor], condition: Tensor, control: Tensor = None, init_t: Optional[int] = None,
                          label_ref_logits: Optional[Tensor] = None) -> dict:
        ...
        for t in t_values:
            ...
            t_control = control[t-1]
            # Auxiliary values
            t_ = torch.full(size=(shape[0],), fill_value=t, device=xt.device)

            # Predict the noise of x_t
            ret = self.unet(xt, condition, t_control, t_.float())
            x0pred = ret["diffusion_out"]
